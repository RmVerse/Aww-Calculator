...
inline void static __SetPos(int x,int y);
...
void Rmdust_System_Console_Graphic_Animation_LeaveMessage(char* Message) {
  ...
  for (size_t Index = 0;Index < MessageLength;Index += 1) {
    ...
    for (size_t InternalIndex = 0;InternalIndex < MessageLength;InternalIndex += 1) {
      if (MessageIn[Index] < MessageIn[InternalIndex]) {
        MessageIndexTemp = InternalIndex;
      }
    }

    if (MessageIndexTemp != Index) {
      MessageIn[MessageIndexTemp] = ' ';
    }
...
void Rmdust_System_Console_Graphic_Text_Window_SquareLine(char* Message) {
  ...
  __Println();
  printf("%c" , __Text);
  for (unsigned short int Count = __WindowWidthCenter - MessageLength/2;Count > 0;Count -= 1) {
    printf("%c" , __Space);
  }
  
  printf("%s" , Message);
  for (unsigned short int Count = __WindowWidthEnd - 1 - (MessageLength % 2 == 0 ? MessageLength / 2 - 1 : MessageLength / 2);Count > 0 ;Count -= 1) {
    printf("%c" , __Space);
  }
  printf("%c" , __Text);
  __Printf();
